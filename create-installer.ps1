# =============================================================================
# OpenMinis PC - Installer Generator v4
...
if (-not $SourceDir) { $SourceDir = Join-Path $scriptDir "release\OpenMinis" }
if (-not $OutputExe) { $OutputExe = Join-Path $scriptDir "release\OpenMinis-Setup.exe" }

Write-Host "=== OpenMinis Setup Builder ===" -ForegroundColor Cyan
...
$zipPath = Join-Path $env:TEMP "openminis-package.zip"
...
class OpenMinisSetup
{
    static string InstallDir
    {
        get { return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "OpenMinis"); }
    }
    ...
    {
        get { return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.StartMenu), "Programs", "OpenMinis"); }
    }
    ...
    static void Main()
    {
        Console.Title = "OpenMinis Setup";
        Console.ForegroundColor = ConsoleColor.Cyan;
        Console.WriteLine("OpenMinis PC Setup");
        ...
        try
        {
            ...
            Console.WriteLine("Extracting " + (zipSize / 1024 / 1024) + " MB...");

            string zipFile = Path.Combine(Path.GetTempPath(), "openminis.zip");
            using (var fs = new FileStream(zipFile, FileMode.Create))
            {
                fs.Write(allBytes, (int)zipOffset, (int)zipSize);
            }

            foreach (var p in System.Diagnostics.Process.GetProcessesByName("OpenMinis"))
            {
                try { p.Kill(); } catch { }
            }
            ...
            CreateShortcut(Path.Combine(DesktopDir, "OpenMinis.lnk"),
                Path.Combine(InstallDir, "OpenMinis.exe"), InstallDir,
                "OpenMinis - Your private AI agent", IconPath);
            // Fallback .url file
            string urlPath = Path.Combine(DesktopDir, "OpenMinis.url");
            File.WriteAllText(urlPath, "[InternetShortcut]\nURL=file:///" +
                Path.Combine(InstallDir, "OpenMinis.exe").Replace("\\", "/") +
                "\nIconFile=" + IconPath.Replace("\\", "/") + "\nIconIndex=0");

            Directory.CreateDirectory(StartMenuDir);
            CreateShortcut(Path.Combine(StartMenuDir, "OpenMinis.lnk"),
                Path.Combine(InstallDir, "OpenMinis.exe"), InstallDir,
                "OpenMinis - Your private AI agent", IconPath);

            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("Done! Desktop shortcut created.");
            Console.ResetColor();
            System.Diagnostics.Process.Start(Path.Combine(InstallDir, "OpenMinis.exe"));
        }
        catch (Exception ex)
        {
...
$csPath = Join-Path $env:TEMP "OpenMinisSetup.cs"
...
$stubExe = Join-Path $env:TEMP "OpenMinisStub.exe"
$iconPath = Join-Path $scriptDir "resources\icon.ico"
$compArgs = "/target:exe /win32icon:`"$iconPath`" /out:`"$stubExe`" /reference:System.IO.Compression.FileSystem.dll `"$csPath`""
...
Write-Host ""
Write-Host "Run this EXE on any Windows machine to install OpenMinis." -ForegroundColor Cyan
Write-Host "It will extract to %LOCALAPPDATA%\OpenMinis and create desktop/start-menu shortcuts." -ForegroundColor Cyan
